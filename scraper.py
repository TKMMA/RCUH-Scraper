...
def clean_text(text):
    ...
    # List of terms to stay in ALL CAPS
    caps_list = ["i", "ii", "iii", "iv", "v", "vi", "vii", "viii", "ix", "x", "dlnr", "rcuh", "dar"]
...
def parse_salary(desc):
    ...
    text = re.sub('<[^<]+?>', ' ', desc)
    # Matches patterns like $3,000 to $4,000 per month or $50,000 per year
    ...
    if match:
        try:
            ...
            if 'hr' in unit or 'hour' in unit: return amount * 2080 # Std work year
        except: return None
    return None
    ...
    try:
        ...
        for row in soup.find_all('tr'):
            ...
            if cols and "RCUH" in cols[0].text:
                ...
                # Try to find the location badge
                loc_badge = row.find('span', class_='badge')
                location = loc_badge.text.strip() if loc_badge else "Hawaii"
                
                # Clean up the title: Remove "RCUH:", the ID#, and leading/trailing punctuation
...
def main():
    ...
    print(f"Successfully scraped {len(data['civil_service'])} civil service and {len(data['rcuh'])} RCUH jobs.")
